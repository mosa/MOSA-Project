...
using Mosa.Platform.x86II.Stages;

namespace Mosa.Platform.x86II
{
	/// <summary>
	...
	sealed class DefaultCallingConvention : ICallingConvention
	{
		...
		private void ReserveStackSizeForCall(Context ctx, int stackSize)
		{
			if (stackSize != 0)
			{
				...
				ctx.AppendInstruction(X86.Mov, new RegisterOperand(architecture.NativeType, GeneralPurposeRegister.EDX), esp);
			}
		}
