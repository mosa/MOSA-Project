...
public class MethodScanner
{
	...
	private void MarkMethodInvoked(MosaMethod method)
	{
		...
		lock (invokedMethods)
		{
			Compiler.LockMonitor.RecordLockWait(lockTimer, invokedMethods);
	...
	public void TypeAllocated(MosaType type, MosaMethod source)
	{
		...
		lock (allocatedTypes)
		{
			Compiler.LockMonitor.RecordLockWait(lockTimer1, allocatedTypes);
		...
		lock (_lock)
		{
			Compiler.LockMonitor.RecordLockWait(lockTimer3, _lock);
	...
	public void InterfaceMethodInvoked(MosaMethod method, MosaMethod source)
	{
		...
		lock (_lock)
		{
			Compiler.LockMonitor.RecordLockWait(lockTimer2, _lock);
			...
			foreach (var type in TypeSystem.AllTypes)
			{
				...
				lock (allocatedTypes)
				{
					Compiler.LockMonitor.RecordLockWait(lockTimer3, allocatedTypes);
	...
	private void ScheduleDerivedMethods(MosaType type, uint slot)
	{
		...
		foreach (var derived in children)
		{
			...
			lock (allocatedTypes)
			{
				Compiler.LockMonitor.RecordLockWait(lockTimer, allocatedTypes);
	...
	public void AccessedField(MosaField field)
	{
		...
		lock (accessedFields)
		{
			Compiler.LockMonitor.RecordLockWait(lockTimer, accessedFields);
