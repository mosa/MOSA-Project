	...
	public sealed class LinearRegisterAllocator
	{
		...
		private void WalkIntervals()
		{
			...
			while (unhandled.Count != 0)
			{
				...
				TryAllocateFreeRegister(current, active, inactive);
			}

		}

		private bool TryAllocateFreeRegister(ExtendedRegister current, List<ExtendedRegister> active, List<ExtendedRegister> inactive)
		{
			int[] freepos = new int[physicalRegisterCount];

			for (int i = 0; i < physicalRegisterCount; i++)
			{
				bool isFloatingPoint = current.VirtualRegister.IsFloatingPoint;

				if (!isFloatingPoint && integerPositions[i] || isFloatingPoint && floatingPositions[i])
				{
