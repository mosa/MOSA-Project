// Copyright (c) MOSA Project. Licensed under the New BSD License.

using System.Diagnostics;

namespace Mosa.Compiler.Framework.Transforms.Compound;

public abstract class BaseCompoundTransform : BaseTransform
{
	public BaseCompoundTransform(BaseInstruction instruction, TransformType type, bool log = false)
		: base(instruction, type, log)
	{ }

	#region Helpers

	protected static void CopyCompound(Transform transform, Context context, Operand destinationBase, Operand destination, Operand sourceBase, Operand source, Operand operandType)
	{
		var size = transform.MethodCompiler.GetSize(operandType);

		Debug.Assert(size > 0);

		var srcReg = transform.VirtualRegisters.AllocateNativeInteger();
		var dstReg = transform.VirtualRegisters.AllocateNativeInteger();

		context.SetInstruction(Framework.IR.UnstableObjectTracking);

		context.AppendInstruction(transform.AddInstruction, srcReg, sourceBase, source);
		context.AppendInstruction(transform.AddInstruction, dstReg, destinationBase, destination);

		var tmp = transform.VirtualRegisters.AllocateNativeInteger();

		for (var i = 0; i < size;)
		{
			var left = size - i;

			var index = Operand.CreateConstant32(i);

			if (left >= 8 & !transform.Is32BitPlatform)
			{
				// 64bit move
				context.AppendInstruction(Framework.IR.Load64, tmp, srcReg, index);
				context.AppendInstruction(Framework.IR.Store64, null, dstReg, index, tmp);
				i += 8;
				continue;
			}
			else if (left >= 4)
			{
				// 32bit move
				context.AppendInstruction(Framework.IR.Load32, tmp, srcReg, index);
				context.AppendInstruction(Framework.IR.Store32, null, dstReg, index, tmp);
				i += 4;
				continue;
			}
			else if (left >= 2)
			{
				// 16bit move
				context.AppendInstruction(Framework.IR.LoadZeroExtend16x32, tmp, srcReg, index);
				context.AppendInstruction(Framework.IR.Store16, null, dstReg, index, tmp);
				i += 2;
				continue;
			}
			else
			{
				// 8bit move
				context.AppendInstruction(Framework.IR.LoadZeroExtend8x32, tmp, srcReg, index);
				context.AppendInstruction(Framework.IR.Store8, null, dstReg, index, tmp);
				i += 1;
				continue;
			}
		}

		context.AppendInstruction(Framework.IR.StableObjectTracking);
	}

	#endregion Helpers
}
