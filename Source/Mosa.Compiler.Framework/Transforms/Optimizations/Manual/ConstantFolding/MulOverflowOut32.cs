// Copyright (c) MOSA Project. Licensed under the New BSD License.

using Mosa.Compiler.Common;

namespace Mosa.Compiler.Framework.Transforms.Optimizations.Manual.ConstantFolding;

/// <summary>
/// MulOverflowOut32
/// </summary>
public sealed class MulOverflowOut32 : BaseTransform
{
	public static readonly MulOverflowOut32 Instance = new();

	private MulOverflowOut32() : base(IR.MulOverflowOut32, TransformType.Auto | TransformType.Optimization, 100, true)
	{
	}

	public override bool Match(Context context, Transform transform)
	{
		if (!IsResolvedConstant(context.Operand1))
			return false;

		if (!IsResolvedConstant(context.Operand2))
			return false;

		if (IntegerTwiddling.IsMultiplySignedOverflow(context.Operand1.ConstantSigned32, context.Operand2.ConstantSigned32))
			return false;

		return true;
	}

	public override void Transform(Context context, Transform transform)
	{
		var result = context.Result;
		var result2 = context.Result2;

		var t1 = context.Operand1;
		var t2 = context.Operand2;

		var e1 = Operand.CreateConstant(MulSigned32(ToSigned32(t1), ToSigned32(t2)));

		context.SetInstruction(IR.Move32, result, e1);
		context.AppendInstruction(IR.Move32, result2, Operand.Constant32_1);
	}
}
