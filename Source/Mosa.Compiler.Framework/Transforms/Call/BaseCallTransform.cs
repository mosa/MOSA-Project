...
public abstract class BasePlugTransform : BaseTransform
{
	...
	public static void MakeCall(Transform transform, Context context, Operand target, Operand result, List<Operand> operands)
	{
		...
		context.AppendInstruction(Framework.IR.CallDirect, null, target);
	...
	private static void GetReturnValue(Transform transform, Context context, Operand result)
	{
		...
		if (result.IsObject)
		{
			...
			context.AppendInstruction(Framework.IR.Gen, returnLow);
			context.AppendInstruction(Framework.IR.MoveObject, result, returnLow);
		}
		else if (result.IsInt64 && transform.Is32BitPlatform)
		{
			...
			context.AppendInstruction(Framework.IR.Gen, returnLow);
			context.AppendInstruction(Framework.IR.Gen, returnHigh);
			context.AppendInstruction(Framework.IR.To64, result, returnLow, returnHigh);
		}
		...
		else if (result.IsR4)
		{
			...
			context.AppendInstruction(Framework.IR.Gen, returnFP);
			context.AppendInstruction(Framework.IR.MoveR4, result, returnFP);
		}
		else if (result.IsR8)
		{
			var returnFP = transform.PhysicalRegisters.AllocateR8(transform.Architecture.ReturnFloatingPointRegister);
			context.AppendInstruction(Framework.IR.Gen, returnFP);
			context.AppendInstruction(Framework.IR.MoveR8, result, returnFP);
		}
		else if (result.IsValueType)
		{
			context.AppendInstruction(Framework.IR.LoadCompound, result, transform.StackPointer, Operand.Constant32_0);
		}
		else
		{
			var returnLow = transform.PhysicalRegisters.Allocate(result, transform.Architecture.ReturnRegister);
	...
	private static void Push(Transform transform, Context context, Operand operand, uint offset)
	{
		...
		if (operand.IsObject)
		{
			context.AppendInstruction(Framework.IR.StoreObject, null, transform.StackPointer, offsetOperand, operand);
		}
		else if (operand.IsInt32)
		{
			context.AppendInstruction(Framework.IR.Store32, null, transform.StackPointer, offsetOperand, operand);
		}
		else if (operand.IsInt64)
		{
			context.AppendInstruction(Framework.IR.Store64, null, transform.StackPointer, offsetOperand, operand);
		}
		else if (operand.IsR4)
		{
			context.AppendInstruction(Framework.IR.StoreR4, null, transform.StackPointer, offsetOperand, operand);
		}
		else if (operand.IsR8)
		{
			context.AppendInstruction(Framework.IR.StoreR8, null, transform.StackPointer, offsetOperand, operand);
		}
		else if (operand.IsValueType)
		{
			context.AppendInstruction(Framework.IR.StoreCompound, null, transform.StackPointer, offsetOperand, operand);
		}
		else
		{
