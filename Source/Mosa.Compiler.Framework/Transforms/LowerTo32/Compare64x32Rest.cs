...
public sealed class Compare64x32Rest : BaseLowerTo32Transform
{
	...
	public override void Transform(Context context, Transform transform)
	{
		...
		// Success
		newBlocks[2].AppendInstruction(IR.Move32, resultLow, Operand.CreateConstant((uint)1));
		newBlocks[2].AppendInstruction(IR.Jmp, newBlocks[4].Block);

		// Failed
		newBlocks[3].AppendInstruction(IR.Move32, resultLow, Operand.Constant32_0);
		newBlocks[3].AppendInstruction(IR.Jmp, newBlocks[4].Block);

		// Exit
		newBlocks[4].AppendInstruction(IR.Move32, result, resultLow);
