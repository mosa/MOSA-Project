...
namespace Mosa.Compiler.Framework.Intrinsics
{
	...
	public sealed class Store32 : IIntrinsicInternalMethod
	{
		...
		void IIntrinsicInternalMethod.ReplaceIntrinsicCall(Context context, MethodCompiler methodCompiler)
		{
			...
			{
				context.SetInstruction(IRInstruction.StoreInteger32, null, context.Operand1, methodCompiler.ConstantZero, context.Operand2);
			}
			else if (context.OperandCount == 3)
			{
				context.SetInstruction(IRInstruction.StoreInteger32, null, context.Operand1, context.Operand2, context.Operand3);
			}
			else
			{
