...
namespace Mosa.Compiler.Framework.Intrinsics
{
	...
	public sealed class Store16 : IIntrinsicInternalMethod
	{
		...
		void IIntrinsicInternalMethod.ReplaceIntrinsicCall(Context context, MethodCompiler methodCompiler)
		{
			...
			{
				context.SetInstruction(IRInstruction.StoreInteger16, null, context.Operand1, methodCompiler.ConstantZero, context.Operand2);
			}
			else if (context.OperandCount == 3)
			{
				context.SetInstruction(IRInstruction.StoreInteger16, null, context.Operand1, context.Operand2, context.Operand3);
			}
			else
			{
