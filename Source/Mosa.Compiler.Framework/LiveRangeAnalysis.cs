	...
	public sealed class LiveRangeAnalysis
	{
		...
		private void CollectVirtualRegisters()
		{
			foreach (var temp in methodCompiler.StackLayout.Stack)
			{
				...
				{
					temporaryLiveRanges[temp.Index] = new TemporaryLiveRange(temp);
				}
			}
		}
		...
		private void CalculateLiveRanges(TemporaryLiveRange temp)
		{
			...
			{
				int startContextIndex = temp.Operand.Definitions[0];
				int endContextIndex = startContextIndex;

				foreach (var use in uses)
				{
					if (use > endContextIndex)
					{
						endContextIndex = use;
					}
				}
