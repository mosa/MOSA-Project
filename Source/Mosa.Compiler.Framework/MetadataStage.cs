		...
		private void CreateAssemblyListTable()
		{
			...
			using (Stream stream = linker.Allocate(assemblyListSymbol, SectionKind.ROData, 0, typeLayout.NativePointerAlignment))
			{
				using (EndianAwareBinaryWriter writer = new EndianAwareBinaryWriter(stream, true)) // FIXME
				{
					...
					{
						linker.Link(LinkType.AbsoluteAddress | LinkType.I4, assemblyListSymbol, (int)writer.Position, 0, module.Name + "$atable", IntPtr.Zero);
				...
				using (EndianAwareBinaryWriter writer = new EndianAwareBinaryWriter(stream, true)) // FIXME
				{
					...
					foreach (var type in typeModule.GetAllTypes())
					{
						if (!type.IsModule && !(type.Module is InternalTypeModule))
							linker.Link(LinkType.AbsoluteAddress | LinkType.I4, assemblyTableSymbol, (int)writer.Position, 0, type.FullName + @"$dtable", IntPtr.Zero);
		...
		private void CreateTypeDefinitionTable(RuntimeType type, string assemblySymbol)
		{
			...
			using (Stream stream = linker.Allocate(typeTableSymbol, SectionKind.ROData, 0, typeLayout.NativePointerAlignment))
			{
				using (EndianAwareBinaryWriter writer = new EndianAwareBinaryWriter(stream, true)) // FIXME
				{
					...
					// 3. Pointer to Name
					linker.Link(LinkType.AbsoluteAddress | LinkType.I4, typeTableSymbol, (int)writer.Position, 0, typeNameSymbol, IntPtr.Zero);
					writer.Position += typeLayout.NativePointerSize;

					// 4. Pointer to Assembly Definition
					linker.Link(LinkType.AbsoluteAddress | LinkType.I4, typeTableSymbol, (int)writer.Position, 0, assemblySymbol, IntPtr.Zero);
					writer.Position += typeLayout.NativePointerSize;
