...
namespace System.Collections
{
	...
	public class Queue : ICollection, ICloneable
	{
		...
		{
			if (capacity < 0)
				throw new ArgumentOutOfRangeException(nameof(capacity), "Need positive number");
			if (!(growFactor >= 1.0 && growFactor <= 10.0))
				throw new ArgumentOutOfRangeException(nameof(growFactor), "Argument out of range for queue grow factor, must be between 1 and 10");
		...
		public virtual void CopyTo(Array array, int index)
		{
			...
			if (index < 0)
				throw new ArgumentOutOfRangeException(nameof(index), "Argument out of range for index");
		...
		private sealed class QueueEnumerator : IEnumerator, ICloneable
		{
			...
			{
				if (_version != _q._version) throw new InvalidOperationException("Invalid operation, version enumeration failed");
