...
namespace Mosa.Tools.Permutations
{
	...
	{
		public static string Type = "sbyte";
		...
		public static IList<string> GetAllPermutations()
		{
			...
			list.AddIfNew(sbyte.MinValue.ToString());
			list.AddIfNew(sbyte.MaxValue.ToString());
			list.AddIfNew((sbyte.MinValue + 1).ToString());
			list.AddIfNew((sbyte.MaxValue - 1).ToString());

			//list.AddIfNew(17.ToString());
			//list.AddIfNew(123.ToString());

			// power of two numbers
			//list.AddIfNew(Power2.GetPowerTwos(0, (ulong)sbyte.MaxValue));

			// a few prime numbers
			//list.AddIfNew(Prime.GetSomePrimes(0, (ulong)sbyte.MaxValue));

			// Get negatives
			list.AddIfNew(Negative.GetNegatives(sbyte.MinValue, list));
