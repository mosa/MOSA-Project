...
namespace Mosa.Tools.Permutations
{
	...
	{
		public static string Type = "ushort";
		...
		public static IList<string> GetAllPermutations()
		{
			...
			list.AddIfNew(ushort.MinValue.ToString());
			list.AddIfNew(ushort.MaxValue.ToString());
			list.AddIfNew((ushort.MinValue + 1).ToString());
			list.AddIfNew((ushort.MaxValue - 1).ToString());

			//list.AddIfNew(17.ToString());
			//list.AddIfNew(123.ToString());

			// power of two numbers
			//list.AddIfNew(Power2.GetPowerTwos(ushort.MinValue, ushort.MaxValue));

			// a few prime numbers
			//list.AddIfNew(Prime.GetSomePrimes(ushort.MinValue, ushort.MaxValue));
