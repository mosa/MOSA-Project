...
internal class ClrMethodScanner : MethodScanner
{
	...
	public ClrMethodScanner(Compiler compiler)
	{
		Compiler = compiler;
	...
	public override void TypeAllocated(MosaType type, MosaMethod source)
	{
		if (!IsEnabled)
			return;
	...
	public override void MethodInvoked(MosaMethod method, MosaMethod source)
	{
		if (!IsEnabled)
			return;
	...
	public override void MethodDirectInvoked(MosaMethod method, MosaMethod source)
	{
		if (!IsEnabled)
			return;
	...
	public override void InterfaceMethodInvoked(MosaMethod method, MosaMethod source)
	{
		if (!IsEnabled)
			return;
	...
	public override void AccessedField(MosaField field)
	{
		if (!IsEnabled)
			return;
	...
	public override bool IsFieldAccessed(MosaField field)
	{
		if (!IsEnabled)
			return true; // always
	...
	public override bool IsTypeAllocated(MosaType type)
	{
		if (!IsEnabled)
			return true; // always
	...
	public override bool IsMethodInvoked(MosaMethod method)
	{
		if (!IsEnabled)
			return true; // always
	...
	public override void MoreLogInfo()
	{
		foreach (var type in TypeSystem.AllTypes)
		{
			if (type.IsModule)
