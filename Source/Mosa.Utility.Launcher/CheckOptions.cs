...
public static class CheckOptions
{
	public static string Verify(MosaSettings settings)
	{
		...
		return settings.Emulator switch
		{
			...
			"vmware" when settings.ImageFormat == "img" => "VMware does not support the IMG image format",
			"vmware" when settings.ImageFormat == "vdi" => "VMware does not support the VHD image format",
