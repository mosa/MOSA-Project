...
namespace Mosa.Runtime.Vm
{
	...
	public sealed class DefaultModuleTypeSystem : IModuleTypeSystem, IModuleTypeSystemInternalList
	{
		...
		{			
			switch (token & TokenTypes.TableMask)
			{
				...
				case TokenTypes.TypeDef:
					...
					return typeDefRowIndex == 0 ? null : types[typeDefRowIndex - 1];
		...
		{
			foreach (RuntimeType type in this.types)
			{
		...
		/// <returns></returns>
		RuntimeType ResolveSignatureType(SigType sigType, TokenTypes token)
		{
			switch (sigType.Type)
			{
		...
		private void LoadTypes()
		{
			...
			typeDefRow = metadata.ReadTypeDefRow(TokenTypes.TypeDef + 1);
			...
			switch (token & TokenTypes.TableMask)
			{
				case TokenTypes.TypeSpec:
				{
					...
					if ((genericInstSigType.BaseType.Token & TokenTypes.TableMask) == TokenTypes.TypeRef)
					{
						...
						{
							TypeRefRow nestedTypeRefRow = metadata.ReadTypeRefRow(resolutionScopeIdx);
							resolutionScopeIdx = nestedTypeRefRow.ResolutionScopeIdx;	
						}
		...
		private RuntimeType ResolveTypeSpec(TokenTypes typeSpecToken)
		{
			...
			if (typeSpecs[typeSpecIndex] == null)
			{
				...
				if (signature.Type.Type == CilElementType.Class || signature.Type.Type == CilElementType.ValueType || signature.Type.Type == CilElementType.GenericInst)
				{
					...
					if (genericSig != null)
					{
						RuntimeType genericType = ResolveSignatureType(genericSig, typeSpecToken);
