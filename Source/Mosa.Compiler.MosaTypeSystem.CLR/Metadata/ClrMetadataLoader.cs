...
internal class ClrMetadataLoader
{
	...
	private MosaType Load(TypeSig typeSig)
	{
		...
		switch (typeSig.ElementType)
		{
			case ElementType.Ptr:
				...
				if (result == null)
					throw new InvalidOperationCompilerException("Unmanaged pointer of element type is null!");
			...
			case ElementType.SZArray:
				...
				if (result == null)
					throw new InvalidOperationCompilerException("SZ array of element type is null!");
				...
				using (var arrayType = metadata.Controller.MutateType(result))
				{
					...
					if (!typeSig.Next.HasOpenGenericParameter())
					{
						...
						if (szHelperMethods == null)
							throw new InvalidOperationCompilerException("szHelperMethods array is null!");
			...
			case ElementType.Array:
				...
				if (result == null)
					throw new InvalidOperationCompilerException("Array of element type is null!");
	...
	private MosaType LoadGenericTypeInstanceSig(GenericInstSig typeSig)
	{
		...
		using (var resultType = metadata.Controller.MutateType(result))
		{
			...
			for (int i = 0; i < result.Properties.Count; i++)
			{
				...
				if (propertySig == null)
					throw new InvalidOperationCompilerException("Property signature is null!");
	...
	public MosaMethod LoadGenericMethodInstance(IMethodDefOrRef method, IList<TypeSig> genericArguments, GenericArgumentResolver resolver)
	{
		...
		if (resolvedType == null)
			throw new InvalidOperationCompilerException("Resolved type of method's declaring type signature is null!");
		...
		foreach (var genericArg in genericArguments)
		{
			...
			if (resolvedGenericArg == null)
				throw new InvalidOperationCompilerException("Resolved generic argument is null!");
