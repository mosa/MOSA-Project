...
namespace Mosa.Platform.x86.Intrinsic
{
	...
	public sealed class InvokeDelegateWithReturn : IIntrinsicMethod
	{
		...
		void IIntrinsicMethod.ReplaceIntrinsicCall(Context context, ITypeSystem typeSystem, IList<RuntimeParameter> parameters)
		{
			...
			context.SetInstruction(OpCodes.Instruction.SubInstruction, esp, new ConstantOperand(BuiltInSigType.IntPtr, parameters.Count * 4));
			context.AppendInstruction(OpCodes.Instruction.MovInstruction, edx, esp);

			var size = parameters.Count * 4;
			foreach (var parameter in parameters)
			{
				context.AppendInstruction(OpCodes.Instruction.MovInstruction, new MemoryOperand(BuiltInSigType.IntPtr, edx.Register, new IntPtr(size - 4)), new MemoryOperand(BuiltInSigType.IntPtr, ebp.Register, new IntPtr(size + 8)));
				size -= 4;
			}	
			...
			context.AppendInstruction(OpCodes.Instruction.AddInstruction, esp, new ConstantOperand(BuiltInSigType.IntPtr, parameters.Count * 4));
