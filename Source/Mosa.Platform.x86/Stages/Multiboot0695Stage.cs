		...
		private void WriteMultibootHeader()
		{
			...
			using (Stream stream = linker.Allocate(MultibootHeaderSymbolName, SectionKind.Text, 64, 4))
			{
				using (BinaryWriter bw = new BinaryWriter(stream, Encoding.ASCII))
				{
					...
					linker.Link(LinkType.AbsoluteAddress | LinkType.I4, BuildInPatch.I4, MultibootHeaderSymbolName, (int)stream.Position, 0, @"Mosa.Tools.Compiler.LinkerGenerated.<$>MultibootInit()", 0);
					bw.Write((int)0);
