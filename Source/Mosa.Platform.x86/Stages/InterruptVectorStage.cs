		...
		private void CreateInterruptVectors()
		{
			...
			for (int i = 0; i <= 255; i++)
			{
				...
				if (i <= 7 || i >= 16 | i == 9) // For IRQ 8, 10, 11, 12, 13, 14 the cpu will automatically pushed the error code
					ctx.AppendInstruction(X86.Push, null, Operand.CreateConstant(BuiltInSigType.UInt32, 0x0));
				ctx.AppendInstruction(X86.Push, null, Operand.CreateConstant(BuiltInSigType.UInt32, (byte)i));
				ctx.AppendInstruction(X86.Pushad);
				ctx.AppendInstruction(X86.Call, null, interruptMethod);
				ctx.AppendInstruction(X86.Popad);
				ctx.AppendInstruction(X86.Add, esp, Operand.CreateConstant(BuiltInSigType.UInt32, 0x08));
