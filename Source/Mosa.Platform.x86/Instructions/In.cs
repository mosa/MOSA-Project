...
namespace Mosa.Platform.x86.Instructions
{
	...
	public sealed class In : TwoOperandInstruction
	{
		...
		protected override OpCode ComputeOpCode(Operand destination, Operand source, Operand empty)
		{
			...
			if (IsByte(source))
			{
				if ((destination is DefinedRegisterOperand) && (source is ConstantOperand)) return R_C_8;
				if ((destination is DefinedRegisterOperand) && (source is DefinedRegisterOperand)) return R_R_8;
			}
			else
			{
				if ((destination is DefinedRegisterOperand) && (source is ConstantOperand)) return R_C_32;
				if ((destination is DefinedRegisterOperand) && (source is DefinedRegisterOperand)) return R_R_32;
			}
