...
namespace Mosa.DeviceSystem.Network
{
	public static unsafe class Ethernet
	{
		...
		public static void HandlePacket(INetworkDevice device, byte* Data, int Length)
		{
			EthernetHeader* hdr = (EthernetHeader*)Data;
			Data += sizeof(EthernetHeader);
			Length -= sizeof(EthernetHeader);
			if (SwapLeftRight(hdr->EthernetType) == (ushort)EthernetType.ARP) ARP.HandlePacket(device, Data, Length);
			if (SwapLeftRight(hdr->EthernetType) == (ushort)EthernetType.IPv4) IPv4.HandlePacket(device, Data, Length);
		}

		internal static void SendPacket(INetworkDevice device, byte[] DestMAC, ushort Type, void* Data, int Length)
		{
			var p = HAL.AllocateMemory((uint)(Length + sizeof(EthernetHeader)), 0).Address;
			...
			Internal.MemoryCopy(p + (uint)sizeof(EthernetHeader), (Pointer)Data, (uint)Length);
			device.SendPacket(p, (uint)(Length + sizeof(EthernetHeader)));
		}
